...
def event_processor(request):
    try:
        ...
        # Обработка события ONCRMLEADADD
        if event == "ONCRMLEADADD":
            from thoth.telegram.models import User, Reklama_Campaign
            print(data)
            id = data.get("data[FIELDS][ID]")
            lead_data = call_method(appinstance, "crm.lead.get", {"id": id})
            result = lead_data['result']['IM'][0]['VALUE'].split('|')
            if result[1] == 'thoth_telegram':
                lead_id = result[3]
                utm_source = User.objects.get(user_id=lead_id).source_id
                utm_campaign = User.objects.get(user_id=lead_id).campaign_id
                utm_content = str(Reklama_Campaign.objects.get(id=utm_campaign).adv_id) if utm_campaign else None
                payload = {
                    "id": id,
                    "fields": {
                        "UF_CRM_LEAD_ID": f"tg_{lead_id}",
                ...
                print(payload)
                call_method(appinstance, "crm.lead.update", payload)

            return Response(
                {"status": "ONCRMLEADADD event processed"},
        ...
        # Обработка события ONIMCONNECTORMESSAGEADD
            ...
            # If Telegram connector
            elif connector == 'thoth_telegram':
                import asyncio
                from aiogram.exceptions import TelegramForbiddenError, TelegramEntityTooLarge
                from thoth.telegram.models import Bot
                from thoth.telegram.utils import send_message, send_photo, send_video, send_document
                line_id = Line.objects.get(line_id=line)
                token = Bot.objects.get(line_id=line_id).bot_token
                user_id = data.get("data[MESSAGES][0][chat][id]")
                chat_id = data.get("data[MESSAGES][0][im][chat_id]")
                ...
                try:
                    if files:
                        for file in files:
                            if file["type"] == "image":
                                asyncio.run(send_photo(token, user_id, file['link']))
                            elif file["type"] == "video":
                                asyncio.run(send_video(token, user_id, file['link'], file["name"]))
                            elif file["type"] in ["file", "audio"]:
                                asyncio.run(send_document(token, user_id, file['link'], file["name"]))
                    else:
                        if 'Отказ: пользователь заблокировал бота!' in text:
                            pass
                        else:
                            asyncio.run(send_message(token, user_id, text))
                    call_method(appinstance, "imconnector.send.status.delivery", payload)
                except TelegramForbiddenError as e:
                    print(e.message)
                    user_id = data.get("data[MESSAGES][0][message][user_id]")
                    result = call_method(appinstance, "imopenlines.dialog.get", {"CHAT_ID": chat_id})
                    lead_id = result['result']['entity_data_2'].split("|")[1]
                    ...
                    payload = {
                        ...
                        "CHAT_ID": chat_id,
                        "LINE": line,
                        "MESSAGE": "Отказ: пользователь заблокировал бота!"
                    }

                    call_method(appinstance, "imopenlines.crm.message.add", payload)
